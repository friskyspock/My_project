...
double_t delR(double_t eta1, double_t eta2,double_t phi1, double_t phi2){
	/*
	   ...
	   is more than pi, then it must be subtracted by 2*pi as the range of 
	   eta and phi is [0,pi]
	   */
...
tuple <double_t,double_t> invFL(vector<TLorentzVector> leptons, 
	...
	for(Int_t i=0;i<leptons.size();i++){
		for(Int_t j=0;j<fatjets.size();j++){
			...
			if(max_invFL < fabs((leptons[i] + fatjets[j]).M()));{
				max_invFL = fabs((leptons[i] + fatjets[j]).M());
...
int main(int argc,char* argv[]){
	...
	/*
	   ...
	   - inlv    : Reconstructed Invariant Mass of Neutrino and Leading Lepton
	...
	myfile << "# Total Entries : " << allEntries << "\n";
	myfile << "ptl,etal,energyl,ptj,etaj,energyj,massj,mjj,rjj,";
	myfile << "rjl,met,inlv,n21_1,n21_2,n32_1,n32_2,infl,drfl\n";
	...
	for(Long64_t entry = 0; entry < allEntries; entry++){
		...
		double_t inFL,drFL;
		tie(inFL,drFL) = invFL(lepton_set,tower_set);
		myfile << inFL << "," << drFL << "\n";

	}
	myfile.close();
	cout << "The number of events left are : " << no_events << endl;
	return 0;
}
